...
import {MenuItems, Menu, MainMenuItems} from '../../shared/menu-items/menu-items';
...
export class AdminComponent implements OnInit, NotificationsUpdater {
  
  @ViewChild('notificator') notificator: NotificationsComponent;

  refreshNotifications() {
    this.notificator.refresh();
  }

  get usuario(): ApplicationUser {
    return this.authService.currentUser;
  }

  hasPermittedRole(permittedRoles: string[]): boolean {
    for (let permittedRole of permittedRoles) {
      if (this.usuario.roles.indexOf(permittedRole) != -1)
        return true;
    }
    return false;
  }

  anyItemHasPermittedRole(items: MainMenuItems[]): boolean {
    for (let item of items) {
      if (this.hasPermittedRole(item.permittedRoles))
        return true;
    }
    return false;
  }
  ...
  ngOnInit() {
    var self = this;
    if (this.hasPermittedRole(["LABORATORIO"])) {
      this.isLaboratory = true;
      this.authService.getLaboratoryUser().subscribe(x => {
        let laboratoryUser: LaboratoryUser = x as LaboratoryUser;
        if (laboratoryUser.laboratory.color){
          document.querySelector("#user-background").setAttribute("style", "background:" + laboratoryUser.laboratory.color);
          document.querySelector(".pcoded-header").setAttribute("style", "background:" + laboratoryUser.laboratory.color);
        }
        if (laboratoryUser.laboratory.iconBytes)
          document.querySelector("#logo-icon").setAttribute("src", 'data:image/png;base64,'+laboratoryUser.laboratory.iconBytes);
      });
    }
  }
